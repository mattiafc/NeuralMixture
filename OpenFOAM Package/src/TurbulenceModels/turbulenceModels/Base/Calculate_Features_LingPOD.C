...
eta_list.append(Pk_  / ( Pk_  + epsilon_  + epsP));
...
for (int i = 0; i < size; ++i) {
    for (int j = 0; j < size; ++j) {
        ...
        forAll(this->mesh_.cells(), celli)
        	{
        		innerProduct += eta_list[i]()[celli] * eta_list[j]()[celli];
        	}
        
        if (Pstream::nProcs() > 1) {
		    // inner product for parallel execution
		    globalSum = 0.0;
			MPI_Allreduce(&innerProduct, &globalSum, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
		} 
		else {
			// inner product serial execution
			globalSum = innerProduct;
		}
        
        
        A[i][j] = globalSum; //fvc::domainIntegrate( eta_list[i]() * eta_list[j]() ).value(); 
    }
}
