...
eta_list.append((sqr(normFrob_Oij) - sqr(normFrob_Sij)) /  max(mag(sqr(normFrob_Sij) + sqr(normFrob_Oij)) , epsS));
...
eta_list.append(mag(gradP) / max ( mag(gradP) + this->rho_ * mag(U_diag_gradU) , eps7));
