...
dimensionedScalar epsP("epsP",dimensionSet(0,2,-3,0,0,0,0),1e-0);
dimensionedScalar eps7("eps7",dimensionSet(0,1,-2,0,0,0,0),1e-0);
dimensionedScalar epsS("epsS",dimensionSet(0,0,-2,0,0,0,0),1e-0);
...
//eta_list.append(min(sqrt(k_)* wallDist(this->mesh_).y() /(50.*this->nu()), 2.));

//Info << "\n eta_4 Pressure gradient along streamline" << nl << endl;
eta_list.append( scalar(1) - mag(U)*mag(gradP) /  ( mag(U&gradP) + mag(U)*mag(gradP) + epsP));
...
eta_list.append(( mag(this->Sij_dim()) * k_ ) / (  (mag(this->Sij_dim()) * k_) + epsilon_));
...
eta_list.append( mag(U_UgradU) / ( mag(U_UgradU) + mag(U_gradU_gradUT_U) * mag(U) + epsP));

//Info << "\n eta_9 Ratio of convection to production k" << nl << endl;
eta_list.append(Ugradk / ( Ugradk + mag(Rall_ && this->Sij_dim())  + epsP));
...
eta_list.append( Pk_  / ( Pk_  + epsilon_  + 0.001*epsP));
